...
void Herkulex::Moving(const int* goal, const int* playTime,
		const uint8_t* color) {
...
// playTime : [msec]
void Herkulex::MovingSync(const int* goal, const uint8_t* color,
		const int playTime) {
	// servo 16 ea
...
// playTime : [msec]
void Herkulex::MovingOne(const int* goal, const int playTime,
		const uint8_t* color) {
	// servo 16 ea
	const uint8_t packetSize = 72;	// 8 + 4 * 16
	uint8_t checkSum1 = 0;
...
void Herkulex::littleEndian(uint8_t* lsb, uint8_t* msb, const int data) {
	*msb = (0xFF00 & data) >> 8;
	*lsb = (0x00FF & data);
}
...
int32_t Herkulex::SetPosition(std::vector<OPRoS::Float64> position,
	...
	int pos[16] = { 0, };
	int playTime[16] = { 0, };
	uint8_t led[16] = { LED_NONE, };

	for (int i = 0; i < 16; ++i) {
		pos[i] = (int) position[i].data;
		playTime[i] = (int) time[i];
		led[i] = LED_NONE;
	}
...
int32_t Herkulex::SetPosition(std::vector<OPRoS::Float64> position,
		std::vector<uint32_t> time, std::vector<uint8_t> color) {
#ifdef DEBUG
	std::cout << "Herkulex::SetPosition()" << std::endl;
#endif
	...
	for (int i = 0; i < 16; ++i) {
		if (position[i].data > 1002)
			pos[i] = 1002;
		else if (position[i] < 21)
			pos[i] = 21;
		else
			pos[i] = (int) position[i].data;
...
int32_t Herkulex::SetLED(std::vector<uint8_t> color) {
#ifdef DEBUG
	std::cout << "Herkulex::SetLED()" << std::endl;
#endif
	uint8_t id[16] = { 0, };
	uint8_t led[16] = { 0, };
