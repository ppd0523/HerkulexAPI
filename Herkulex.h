...
private:
	...
	uint8_t CheckSum1(uint8_t* packet, int size);
...
private:
	...
	void littleEndian(uint8_t* lsb, uint8_t* msb, int data);
	void Moving(int* goal, int* playTime, uint8_t* color);
	void MovingSync(int* goal, uint8_t* color, int playTime);
	void MovingOne(int* goal, int playTime, uint8_t* color);
	// playTime : 0 ~ 2856 [msce]
	void SetLED(uint8_t servoID, uint8_t color);
